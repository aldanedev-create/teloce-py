    ...
    def _generate_runtime(
        ...
        runtime = [
            ...
            *([f'import {{ __safeEvaluate, __runEventExpression, __setSafePath, __createReactive, __patch }} from {json.dumps(shared_runtime_import)};'] if shared_runtime_import else []),
            ...
            'const __isDangerousUrl = value => /^(?:javascript|vbscript|data|file):/.test(String(value ?? "").trim().toLowerCase());',
            ...
            *(['  const __flipSnapshot = root => { if (!root) return []; return Array.from(root.children || []).filter(node => node.dataset && node.dataset.teloceAnimate === "flip").map(node => [node, node.getBoundingClientRect()]); };'] if uses_flip else []),
            ...
            '  const __resolveNestedIf = (text, scope) => { let previous; do { previous = text; text = text.replace(/<if\\s+[^>]*?(?:condition|test)="([^"]*)"[^>]*>([\\s\\S]*?)(?:<else>([\\s\\S]*?))?<\\/if>/, (_, test, yes, no) => __evaluate(test, scope) ? yes : (no || "")); } while (text !== previous); return text; };',
            ...
            '  { let previous; do { previous = output; output = output.replace(/<if\\s+(?:condition|test)="([^"]*)">([\\s\\S]*?)(?:<else>([\\s\\S]*?))?<\\/if>/, (_, test, yes, no) => __evaluate(test, state) ? yes : (no || "")); } while (output !== previous); }',
